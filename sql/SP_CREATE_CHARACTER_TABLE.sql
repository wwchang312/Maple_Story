...
-- 	1-2) 업적 정보 조회 테이블
	...
	/*
	 ...
	 캐릭터 정보 조회 테이블 중, [date] 컬럼이 존재하는 테이블은 호출 시기에 따른 이력 관리를 위해 별도 히스토리 테이블을 구성.
	 ...
	 8. 캐릭터 성향 정보 조회 
	 9. 캐릭터 어빌리티 정보 조회 
	 10. 캐릭터 장착 장비 정보 조회(캐시 장비 제외)
	 11. 장착 캐시 장비 정보 조회
	 12. 장착 심볼 정보 조회
	 13. 적용 세트 효과 정보 조회
	 14. 장착 헤어, 성형, 피부 정보 조회
	 15. 장착 안드로이드 정보 조회
	 16. 장착 펫 정보 조회
	 17. 스킬 정보 조회
	 18. 장착 링크 스킬 정보 조회
	 19. V매트릭스 정보 조회
	 20. HEXA 코어 정보 조회
	 21. HEXA 매트릭스 설정 HEXA 스탯 정보 조회
	 22. 무릉도장 최고 기록 정보 조회
	 23. 기타 능력치 영향 요소 정보 조회 조회
	 24. 링 익스체인지 스킬 등록 장비 조회
	 */
...
--	캐릭터 기본 정보 히스토리
	SET @sql += N'
	IF OBJECT_ID(N''dbo.character_basic_hist'',''U'') IS NULL
	CREATE TABLE dbo.character_basic_hist (
	[date]					DATETIMEOFFSET,		--조회기준일
	ocid					NVARCHAR(64),		--캐릭터 식별자 (PK용 컬럼, 파라미터의 값을 가져와서 INSERT)
	character_name			NVARCHAR(128),		--캐릭터 명
	world_name				NVARCHAR(64),		--월드 명
	...
	access_flag				NVARCHAR(8),		--최근 7일간 접속 여부
	liberation_quest_clear	NVARCHAR(8),		--해방 퀘스트 완료 여부 (0:미완료, 1:제네시스 무기 해방, 2:데스티니 무기 1차 해방)
	CONSTRAINT pk_character_basic_hist PRIMARY KEY ([date],ocid)
	);';
...
-- 	캐릭터 인기도 정보 히스토리
	SET @sql += N'
	IF OBJECT_ID(N''dbo.character_popularity_hist'',''U'') IS NULL
	CREATE TABLE dbo.character_popularity_hist (
	[date]			DATETIMEOFFSET,				--조회기준일
	ocid			NVARCHAR(64),				--캐릭터 식별자
	popularity		INT,						--캐릭터 인기도
	CONSTRAINT pk_character_popularity_hist PRIMARY KEY ([date],ocid)
	);';
...
--	캐릭터 종합 능력치 정보 히스토리
	SET @sql += N'
	IF OBJECT_ID(N''dbo.character_stat_hist'',''U'') IS NULL
	CREATE TABLE dbo.character_stat_hist (
	[date]				DATETIMEOFFSET,
	ocid				NVARCHAR(64),
	character_class		NVARCHAR(64),
	final_stat			NVARCHAR(MAX),			--현재 스탯 정보
	CONSTRAINT pk_character_stat_hist PRIMARY KEY ([date],ocid)
	);';
...
--	캐릭터 하이퍼 스탯 정보 히스토리
	SET @sql += N'
	IF OBJECT_ID(N''dbo.character_hyper_stat_hist'',''U'') IS NULL
	CREATE TABLE dbo.character_hyper_stat_hist (
	[date]								DATETIMEOFFSET,
	ocid								NVARCHAR(64),
	character_class 					NVARCHAR(64),
	use_preset_no						NVARCHAR(4),		--적용 중인 프리셋 번호
	...
	hyper_stat_preset_3					NVARCHAR(MAX),		--3번 프리셋 하이퍼 스탯 정보
	hyper_stat_preset_3_remain_point	INT					--3번 프리셋 하이퍼 스탯 잔여 포인트
	CONSTRAINT pk_character_hyper_stat_hist PRIMARY KEY ([date],ocid)
	);';
...
--  캐릭터 성향 정보 히스토리
	SET @sql += N'
	IF OBJECT_ID(N''dbo.character_propensity_hist'',''U'') IS NULL
	CREATE TABLE dbo.character_propensity_hist (
	[date]				DATETIMEOFFSET,
	ocid				NVARCHAR(64),
	charisma_level		INT,								--카리스마 레벨
	sensibility_level	INT,								--감성 레벨
	...
	handicraft_level	INT,								--손재주 레벨
	charm_level			INT,								--매력 레벨
	CONSTRAINT pk_character_propensity_hist PRIMARY KEY ([date],ocid)
	);';
...
--	캐릭터 어빌리티 정보 히스토리
	SET @sql += N'
	IF OBJECT_ID(N''dbo.character_ability_hist'',''U'') IS NULL
	CREATE TABLE dbo.character_ability_hist (
	[date]				DATETIMEOFFSET,
	ocid				NVARCHAR(64),
	ability_grade		NVARCHAR(16),						--어빌리티 등급
	ability_info		NVARCHAR(MAX),						--어빌리티 정보
	...
	ability_preset_2	NVARCHAR(MAX),						--어빌리티 2번 프리셋 전체 정보
	ability_preset_3	NVARCHAR(MAX),						--어빌리티 3번 프리셋 전체 정보
	CONSTRAINT pk_character_ability_hist PRIMARY KEY ([date],ocid)
	);';
...
--	캐릭터 장착 장비 정보 히스토리
	SET @sql += N'
	IF OBJECT_ID(N''dbo.character_item_equipment_hist'',''U'') IS NULL
	CREATE TABLE dbo.character_item_equipment_hist (
	[date]						DATETIMEOFFSET,
	ocid						NVARCHAR(64),
	character_gender			NVARCHAR(8),
	character_class				NVARCHAR(64),
	...
	dragon_equipment			NVARCHAR(MAX),				--에반 드래곤 장비 정보(에반인 경우 응답)
	mechanic_equipment			NVARCHAR(MAX),				--메카닉 장비 정보 (메카닉인 경우 응답)
	CONSTRAINT pk_character_item_equipment_hist PRIMARY KEY ([date],ocid)
	);';
...
--	장착 캐시 장비 정보 히스토리
	SET @sql += N'
	IF OBJECT_ID(N''dbo.character_cashitem_equipment_hist'',''U'') IS NULL
	CREATE TABLE dbo.character_cashitem_equipment_hist (
	[date]									DATETIMEOFFSET,
	ocid									NVARCHAR(64),
	character_gender						NVARCHAR(8),
	character_class							NVARCHAR(64),
	...
	additional_cash_item_equipment_preset_2	NVARCHAR(MAX),	--제로인 경우 베타, 엔젤릭버스터인 경우 드레스 업 모드의 2번 코디 프리셋
	additional_cash_item_equipment_preset_3	NVARCHAR(MAX),	--제로인 경우 베타, 엔젤릭버스터인 경우 드레스 업 모드의 3번 코디 프리셋
	CONSTRAINT pk_character_cashitem_equipment_hist	PRIMARY KEY ([date],ocid)
	);';
...
--	장착 심볼 정보 히스토리
	SET @sql += N'
	IF OBJECT_ID(N''dbo.character_symbol_equipment_hist'',''U'') IS NULL
	CREATE TABLE dbo.character_symbol_equipment_hist(
	[date]					DATETIMEOFFSET,
	ocid					NVARCHAR(64),
	character_class			NVARCHAR(64),
	symbol					NVARCHAR(MAX),						--심볼 정보
	CONSTRAINT pk_character_symbol_equipment_hist PRIMARY KEY ([date],ocid)
	);';	
...
--	적용 세트 효과 정보 히스토리
	SET @sql += N'
	IF OBJECT_ID(N''dbo.character_set_effect_hist'',''U'') IS NULL
	CREATE TABLE dbo.character_set_effect_hist (
	[date]			DATETIMEOFFSET,
	ocid			NVARCHAR(64),
	set_effect		NVARCHAR(MAX),							--세트 효과 정보
	CONSTRAINT pk_character_set_effect_hist PRIMARY KEY ([date],ocid)
	);';
...
--	장착 헤어, 성형, 피부 정보 히스토리
	SET @sql += N'
	IF OBJECT_ID(N''dbo.character_beauty_equipment_hist'',''U'') IS NULL
	CREATE TABLE dbo.character_beauty_equipment_hist(
	[date]						DATETIMEOFFSET,
	ocid						NVARCHAR(64),
	character_gender			NVARCHAR(8),
	character_class				NVARCHAR(64),
	...
	additional_character_face	NVARCHAR(MAX),		--캐릭터 성형 정보 (제로인 경우 베타, 엔젤릭버스터인 경우 드레스업)
	additional_character_skin	NVARCHAR(MAX),		--캐릭터 피부 정보 (제로인 경우 베타, 엔젤릭버스터인 경우 드레스업)
	CONSTRAINT pk_character_beauty_equipment_hist PRIMARY KEY ([date],ocid)
	);';
...
--	장착 안드로이드 정보 히스토리
	SET @sql += N'
	IF OBJECT_ID(N''dbo.character_android_equipment_hist'',''U'') IS NULL
	CREATE TABLE dbo.character_android_equipment_hist(
	[date]							DATETIMEOFFSET,
	ocid							NVARCHAR(64),
	android_name					NVARCHAR(32),			--안드로이드 명
	android_nickname				NVARCHAR(32),			--안드로이드 닉네임
	...
	android_preset_2				NVARCHAR(MAX),			--2번 프리셋 안드로이드 정보
	android_preset_3				NVARCHAR(MAX),			--3번 프리셋 안드로이드 정보
	CONSTRAINT pk_character_android_equipment_hist PRIMARY KEY (ocid)
	);'
...
--	장착 펫 정보 히스토리
	SET @sql += N'
	IF OBJECT_ID(N''dbo.character_pet_equipment_hist'',''U'') IS NULL
	CREATE TABLE dbo.character_pet_equipment_hist(
	[date]							DATETIMEOFFSET,
	ocid							NVARCHAR(64),			
	pet_1_name						NVARCHAR(32),			--펫1 명
	pet_1_nickname					NVARCHAR(32),			--펫1 닉네임
	...
	pet_3_appearance				NVARCHAR(1024),			--펫3 외형
	pet_3_appearance_icon			NVARCHAR(1024),			--펫3 외형 아이콘
	CONSTRAINT pk_character_pet_equipment_hist	PRIMARY KEY ([date],ocid)
	);';
...
--	스킬 정보 히스토리
	SET @sql += N'
	IF OBJECT_ID(N''dbo.character_skill_hist'',''U'') IS NULL
	CREATE TABLE dbo.character_skill_hist(
	[date]					DATETIMEOFFSET,
	ocid					NVARCHAR(64),
	character_class			NVARCHAR(64),
	character_skill_grade	NVARCHAR(16),			--스킬 전직 차수
	character_skill			NVARCHAR(MAX),			--스킬 정보
	CONSTRAINT pk_character_skill_hist PRIMARY KEY (ocid,character_skill_grade)
	);';
...
--	장착 링크 스킬 정보 히스토리
	SET @sql += N'
	IF OBJECT_ID(N''dbo.character_link_skill_hist'',''U'') IS NULL
	CREATE TABLE dbo.character_link_skill_hist(
	[date]								DATETIMEOFFSET,
	ocid								NVARCHAR(64),
	character_class						NVARCHAR(64),
	character_link_skill				NVARCHAR(MAX),		--링크 스킬 정보
	...
	character_owned_link_skill_preset_2	NVARCHAR(MAX),		--내 링크 스킬 2번 프리셋 정보	
	character_owned_link_skill_preset_3	NVARCHAR(MAX),		--내 링크 스킬 3번 프리셋 정보	
	CONSTRAINT pk_character_link_skill_hist PRIMARY KEY ([date],ocid)
	);';
...
--	V매트릭스 정보 히스토리
	SET @sql += N'
	IF OBJECT_ID(N''dbo.character_vmatrix_hist'',''U'') IS NULL
	CREATE TABLE dbo.character_vmatrix_hist(
	[date]											DATETIMEOFFSET,
	ocid				 							NVARCHAR(64),
	character_class									NVARCHAR(64),
	character_v_core_equipment						NVARCHAR(MAX),	--V코어 정보
	character_v_matrix_remain_slot_upgrade_point	INT,			--캐릭터 잔여 매트릭스 강화 포인트
	CONSTRAINT pk_character_vmatrix_hist PRIMARY KEY ([date],ocid)
	);';
...
--	HEXA 코어 정보 히스토리
	SET @sql += N'
	IF OBJECT_ID(N''dbo.character_hexamatrix_hist'',''U'') IS NULL
	CREATE TABLE dbo.character_hexamatrix_hist(
	[date]							DATETIMEOFFSET,
	ocid							NVARCHAR(64),
	character_hexa_core_equipment	NVARCHAR(MAX),	--HEXA 코어 정보
	CONSTRAINT pk_character_hexamatrix_hist PRIMARY KEY ([date],ocid)
	);';
...
--	HEXA 매트릭스 설정 HEXA 스탯 정보 히스토리
	SET @sql += N'
	IF OBJECT_ID(N''dbo.character_hexamatrix_stat_hist'',''U'') IS NULL
	CREATE TABLE dbo.character_hexamatrix_stat_hist(
	[date]						DATETIMEOFFSET,
	ocid						NVARCHAR(64),
	character_class				NVARCHAR(64),
	character_hexa_stat_core	NVARCHAR(MAX),	--HEXA 스탯 1 코어 정보
	...
	preset_hexa_stat_core_2		NVARCHAR(MAX),	--프리셋 HEXA 스탯 2 코어 정보
	preset_hexa_stat_core_3		NVARCHAR(MAX),	--프리셋 HEXA 스탯 3 코어 정보
	CONSTRAINT pk_character_hexamatrix_stat_hist	PRIMARY KEY ([date],ocid)
	);';
...
--	무릉도장 최고 기록 정보 히스토리
	SET @sql +=N'
	IF OBJECT_ID(N''dbo.character_dojang_hist'',''U'') IS NULL
	CREATE TABLE dbo.character_dojang_hist(
	[date]				DATETIMEOFFSET,
	ocid				NVARCHAR(64),
	character_class		NVARCHAR(64),
	world_name			NVARCHAR(32),
	...
	date_dojang_record	NVARCHAR(32),			--무릉도장 최고 기록 달성 일
	dojang_best_time	INT						--무릉도장 최고 층수 클리어에 걸린 시간 (초)
	CONSTRAINT pk_character_dojang_hist	PRIMARY KEY ([date],ocid)
	);';

--	기타 능력치 영향 요소 정보 조회 조회
	SET @sql +=N'
...
--	기타 능력치 영향 요소 정보 히스토리
	SET @sql +=N'
	IF OBJECT_ID(N''dbo.character_other_stat_hist'',''U'') IS NULL
	CREATE TABLE dbo.character_other_stat_hist(
	[date]				DATETIMEOFFSET,
	ocid				NVARCHAR(64),
	other_stat			NVARCHAR(MAX),						--능력치에 영향을 주는 요소 및 스탯 정보
	CONSTRAINT pk_character_other_stat_hist	PRIMARY KEY ([date],ocid)
	);';
...
--	링 익스체인지 스킬 등록 장비 히스토리
	SET @sql +=N'
	IF OBJECT_ID(N''dbo.character_ring_exchange_skill_equipment_hist'',''U'') IS NULL
	CREATE TABLE dbo.character_ring_exchange_skill_equipment_hist(
	[date]						DATETIMEOFFSET,
	ocid						NVARCHAR(64),
	character_class				NVARCHAR(64),
	special_ring_exchange_name	NVARCHAR(128),
	special_ring_exchange_level	INT,
	CONSTRAINT pk_character_ring_exchange_skill_equipment_hist	PRIMARY KEY ([date],ocid,special_ring_exchange_name)
	);';
