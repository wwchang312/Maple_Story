...
with DAG(
    dag_id ='DAG_Maple_Character_Android_Equipment_API',
    schedule= [maple_character_info],
    start_date=pendulum.datetime(2026,1,1,tz="Asia/Seoul"),
    tags= ['Maple','Android Equipment'],
    description="캐릭터 안드로이드 장착 아이템 정보 조회",
    catchup=False,
    default_args={
    ...
    maple_character_android_equipment_ETL_task = MapleApiOperator.partial(
        task_id='maple_character_android_equipment_ETL_task',
        data_nm='character/android-equipment').expand(
            ocid= ocid,
            date= view_date
            )
