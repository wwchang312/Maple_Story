...
with DAG(
    ...
    def ocid_list(**kwargs):
        ...
        sql = "SELECT ocid FROM Character_list WHERE ocid NOT IN (SELECT ocid FROM character_persensity );" #이 경우, 1회성에 그치게 되지만, API 호출 제한이 있으므로, 우선 ocid가 DB에 없는 경우만 불러오기 위함
