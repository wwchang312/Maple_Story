...
with DAG(
    dag_id ='dag_api_character_dojang',
    schedule= None,
    start_date=pendulum.datetime(2025,8,1,tz="Asia/Seoul"),
    tags= ['maple','Character dojang'],
    description="캐릭터 무릉 최고 기록 정보",
    catchup=False,
    default_args={
    ...
    def ocid_list(**kwargs):
        ...
        sql = "SELECT ocid FROM character_list WHERE ocid NOT IN (SELECT ocid FROM character_dojang );" #이 경우, 1회성에 그치게 되지만, API 호출 제한이 있으므로, 우선 ocid가 DB에 없는 경우만 불러오기 위함
    ...
    maple_character_dojang_ETL_task = MapleApiOperator.partial(
        task_id='maple_character_dojang_ETL_task',
        data_nm='character/dojang',
