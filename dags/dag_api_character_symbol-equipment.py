...
with DAG(
    dag_id ='dag_api_character_symbol-equipment',
    schedule= None,
    start_date=pendulum.datetime(2025,8,1,tz="Asia/Seoul"),
    tags= ['maple','Maple Symbol equipment'],
    ...
    def ocid_list(**kwargs):
        ...
        sql = "SELECT ocid FROM Character_list WHERE ocid NOT IN (SELECT ocid FROM character_symbol_equipment );" #이 경우, 1회성에 그치게 되지만, API 호출 제한이 있으므로, 우선 ocid가 DB에 없는 경우만 불러오기 위함
    ...
    Maple_Symbol_Equipment_ETL_task = MapleApiOperator.partial(
        ...
        data_nm='character/symbol-equipment',
